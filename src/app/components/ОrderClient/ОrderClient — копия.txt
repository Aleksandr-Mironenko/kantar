...
import styles from './ОrderClient.module.scss'
...
const Order = ({ numberOrder }: { numberOrder: number }) => {
  ...
  const getOrder = async (numberOrder: number) => {
    const request = await fetch("/api/client/orders/search-one-order", {
      method: "POST",
  ...
  const organizerData = openDataOrganizer && userOrganizer.name !== "N/a" && (
    ...
    >
      ...
      {userOrganizer.type_acc !== "noAcc" && userOrganizer.type_acc !== "request" && <p style={{ display: "flex", flexWrap: "wrap" }}>Реферальный код: <b style={{ marginLeft: "5px" }}>{userOrganizer.ref_code ? userOrganizer.ref_code : "не задан"}</b></p>}
