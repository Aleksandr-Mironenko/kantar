...
import styles from "./OrderTableClient.module.scss";
...
export default function OrderTableClient({ searchParams }: { searchParams: { idUserProps?: string, headerPage: string } }) {
  ...
  const getOrders = async (page: number) => {
    ...
    const request = await fetch(
      `/api/client/orders/search-all-orders?${params.toString()}`
    );
    if (!request.ok) {
