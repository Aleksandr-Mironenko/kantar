...
export default function FormPrivate({ alertNotification }: CooperationProps) {
  ...
  const onSubmit = async (data: PrivateIndividualFields) => {
    ...
    if (!response.ok) {
      ...
      alertNotification({
        title: "Заявка на заключение договора отправлена",
        description: "С вами свяжется сотрудник компании с целью согласования даты и места встречи для подписания документов о сотрудничестве"
      });
