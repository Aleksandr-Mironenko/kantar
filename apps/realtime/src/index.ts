...
import { TableOrdersRecord, TableOrdersRecordWithEvent } from '@kantar/shared'
...
const wss = new WebSocketServer({ port: 3001 }) // Сервер WS на порту 3001
const clients = new Set<WebSocket>()            // Множество подключенных клиентов

console.log('Сервер обновления в реальном времени находится на порте 3001]://localhost:3001')

// Буфер для батчинга изменений 
const changeBuffer: TableOrdersRecordWithEvent[] = []
...
supabase.channel('orders-secure')
  .on(
    ...
    payload => {
      ...
      const filtered: TableOrdersRecordWithEvent = {
...
setInterval(() => {
  if (changeBuffer.length === 0) return //если изменений нет — ничего не шлем
